---
theme: seriph
title: "Vibe Research：在大语言模型时代把'做完'的时间还给研究者"
info: |
  任何一个计算或数据驱动的物理项目，都存在一个"智力内核 vs 实现开销"的 80/20 结构：真正的物理判断——一个新想法、一个拟合模型、一个数据异常的物理解释——往往在几天内成形；而把它变成一篇可以投出去的论文，常常需要几个月到几年。这多出来的时间，过去被默认由研究者本人承担：写脚本、调 library、处理图像、批量拟合、画图、套模板、回审稿意见。

  过去一年，agentic 大语言模型（Claude Code、Codex 等）正在改写这条默认规律。报告人在 2025 年 12 月至 2026 年 4 月的四个月里，在 LLM 日常协助下完成 16 篇论文（11 篇已上传 arXiv），覆盖三个子方向。本报告不以产出数字为主题，而是剖析让这一产出成为可能的人机分工模型——我们称之为 Vibe Research：研究者负责问题定义、物理判断与结果验收（Expert Filter），AI 承担从符号推导、数值实现、图表生成到论文撰写的实现链条。

  报告面向以实验为主的研究组，会弱化理论技术细节，重点讨论三件事。第一，Vibe Research 的精确定义、Expert Filter 的作用机制，以及四种典型的失败模式——为什么"无需验证"和"不能使用"两种极端判断都是错的。第二，这套 workflow 在一个子领域能 work 所依赖的四个结构性前提：理论或方法骨架的成熟度、项目代码规模、物理约束的可验证性、benchmark / 参考解的密度。把这四个条件对照到实验工作的典型场景——s-SNOM / near-field 数据处理管线、pump-probe 时间序列的批量拟合、FDTD 与 DFT/BSE 辅助计算、仪器控制与自动化采集脚本、图像批处理与论文图表生成——哪些落在条件之内，哪些尚未成熟，会逐项讨论。第三，作为开放问题，邀请在座各位自行判断：在纳米光学、超快动力学、二维材料光学这些方向里，哪些环节已经可以交给 agentic AI，哪些仍需要等待下一代工具。报告不主张迁移结论，只主张迁移问题。
author: "Jin Lei"
background: none
transition: fade
mdc: true
...
### Vibe Research：在大语言模型时代把"做完"的时间还给研究者
### 一个理论物理子领域的 workflow 样本，兼论对实验光学研究的启示
...
同济大学精密光学工程技术研究所 · 2026 年 6 月 9 日

</div>

<div class="abs-bl m-6 text-sm" style="color: var(--pencil-brown); opacity: 0.7;">
4 个月, 16 篇论文, 11 篇 arXiv, 4 篇发表 (3 PRC + 1 PLB) + 1 篇 PRC 接收
</div>

<div class="abs-br m-6 text-xs" style="color: var(--pencil-red); opacity: 0.9; max-width: 340px; text-align: right;">
⚠ 跨子领域免责声明: 本报告来自一个高度成熟、benchmark 丰富、物理约束强可验证的计算核物理子领域. 请带着"哪些环节能搬到 s-SNOM / pump-probe / 2D 材料的工作流里"的问题来听, 不要带着"这能不能照搬"的想法回去.
</div>

<!--
Central message: 今天不是来讲核反应的, 是来讲一套研究者与 LLM 的分工模型在一个成熟计算物理子领域里是如何 work 的, 以及它里面哪些部分能迁移到实验组的日常工作中.

开场: 各位老师下午好, 谢谢精密光学工程技术研究所的邀请. 我知道这是个以实验为主的组, 所以今天会把核物理的技术细节压到最低, 把 workflow、失败模式、以及"哪些环节能搬到实验工作流"这三件事讲清楚. 过去四个月我在 LLM 日常协助下做了 16 篇论文, 但今天的焦点不是这个数字, 是让这个数字变得可能的那套人机分工, 以及它在 s-SNOM 数据处理、pump-probe 批量拟合、FDTD 扫描、DFT/BSE 辅助计算这类实验组日常里可能意味着什么.

Time: 0:00 to 0:30
过渡: 先给一个跨子领域都能读懂的对照实验.
-->
...
<span class="text-sm" style="color: var(--pencil-brown);">变量只有一个: 谁在写代码. 对实验组听众的翻译: 这相当于把一个单参数 pump-probe 拟合管线, 换成一个同时处理 18 个物理参数、37 路探测通道的全局拟合管线, 从一个季度压到一周以内.</span>
...
# <span style="color: var(--pencil-blue)">●</span> 先把"能不能迁移"的问题形式化
## 跨子领域可迁移的四个前提条件
...
<strong>① 成熟方法骨架</strong><br>
<span style="color: var(--pencil-brown);">理论 / 分析框架稳定, LLM 训练语料里有大量文本可参照. 全新方向没有这个基础.</span>
</div>
...
<span style="color: var(--pencil-brown);">守恒律、对称性、已知极限、benchmark 谱线. LLM 的幻觉在这里被物理挡住.</span>
</div>

<div class="sketch-card sketch-card-green">
<strong>④ 丰富 benchmark / 参考解</strong><br>
<span style="color: var(--pencil-brown);">有"对照答案"——公开数据、标样、文献曲线——可以把 AI 钳在正确范围.</span>
</div>
...
**实验组的对照自检清单 (留给各位老师判断):**<br>
① 你们这个环节的分析/仿真骨架 (FDTD、TDDFT、BSE、near-field 反演、pump-probe 全局拟合…) 是否已稳定?<br>
② 典型代码规模 (采集脚本 / 分析管线 / 仿真 wrapper) 是否在 1k–10k 行?<br>
③ 是否有明确可验证的物理约束 (对称性、sum rule、已知材料标样、已发表谱线)?<br>
④ 是否有足够密度的 benchmark / 参考数据供 AI 对照?
...
<!--
Central message: 这套 workflow 不是万能药. 它对一个子领域 work 与否, 取决于四个结构性条件. 我讲完核反应的案例之后, 你们可以自己对照实验组的各个环节看哪些落在这四个条件之内.

讲: 我不是来说"核物理的经验能直接搬到纳米光学实验". 恰恰相反——不同环节情况差别很大. 所以我换一个讲法, 先把"这种 workflow 能 work 的前提条件"显式化, 四个条件: 成熟方法骨架、代码规模 1k 到 10k、强物理可验证、benchmark 丰富. 直接反应这四个都占满, 所以 work. 实验组里哪些环节也占满? s-SNOM 图像管线? pump-probe 全局拟合? FDTD 参数扫描? DFT/BSE wrapper? 这些问题留给大家, 报告后半段会逐项讨论.
...
**对实验组听众的翻译:** 这是任何数值物理都会遇到的结构性问题 —— **"开放 / 不衰减渐近"与"有限维数值域"不兼容**。在纳米光学里对应 FDTD 需要 PML 吸收边界、near-field 数据需要非平凡的远场外推、BSE 计算需要收敛的 k-grid 与有限 supercell 截断。下面讲的解法只是这个结构问题在核物理里的一个具体答案——重点不在技术细节, 而在"用 vibe research 把它做完"用了几天。
...
**对实验组的要点:** 这种"强可验证物理约束"在 DBMM 的整个开发过程中起到了 **Expert Filter** 的作用 —— LLM 每写一段代码, 都会立刻撞到 benchmark、幺正性、已知极限这些硬约束上, 错一个量级立刻暴露。**没有这层约束, 同样的 workflow 会退化为幻觉生成器**。你们自己的环节里有没有类似的硬约束 (f-sum rule、Kramers-Kronig、已知标样光谱、symmetry-forbidden 过渡), 直接决定 vibe research 能不能在该环节落地。
...
**对实验组听众的翻译:** 这堵墙和你们熟悉的场景同构。把 CDCC 换成"一次全电磁 FDTD / BSE 计算", 把"光学势参数"换成"几何参数 + 介电函数", 把"Bayesian UQ"换成"对实验光谱做全局拟合或贝叶斯反演", 就是同一堵墙 —— 单次仿真几十分钟, 要做几十万次是不可行的。
...
**迁移启发:** 只要你的问题参数变化时解也连续变化 (纳米光学里: 几何尺寸、介电函数参数、激发频率、twist angle、pump-probe 延时…), 这一整类方法都是候选工具 —— 相当于给你的 FDTD / BSE / 散射仿真建一个毫秒级代理。
...
**对实验组听众:** 把"37 耦合通道"读作"37 个探测通道 / 37 条同时拟合的谱线", 把"18 光学势参数"读作"18 维材料 + 几何参数空间", 就是你们做多参数全局拟合、介电函数反演、超快动力学多分量 fitting 问题的同构版本。
...
Time: 23:30 to 25:00
过渡: 如果这真是工作流的改变, 应该能在不同方向上复现. 看 16 篇的 subfield 分布.
-->
...
<div class="sketch-card sketch-card-green">
<div class="font-bold text-sm mb-2">反应理论与机制 (8)</div>
<div style="color: var(--pencil-brown);">

· Coherent Absorption [2601.08245] ⭐ PRC <span class="text-xs">w/ Liu, Ren</span><br>
· Channel couplings redirect [2604.05600] ⭐ PLB <span class="text-xs">w/ Liu, Ren</span><br>
...
· IAV breakup generalization [2604.11226]<br>
· 3-body projectile inclusive breakup [draft]<br>
· Dispersive CDCC effective interaction [draft] <span class="text-xs">w/ Liu, Ren</span>
...
**工作流可泛化.** 不只是一个方向上多发论文, 而是横跨纯理论 (Green function), 计算工具 (GPU solver), 统计方法 (Bayesian calibration), EFT (information geometry). **三个子领域, 五位合作者, 同一条 pipeline.** 对实验组: 这类"技术风格跨度"相当于同一个人同时做 s-SNOM 图像分析、FDTD 仿真、TDDFT 辅助计算、仪器控制脚本和论文初稿——不是样样精通, 是同一条流水线覆盖各环节。
...
<!--
Central message: 16 篇不是在一个方向上连发 16 次, 而是横跨三个子领域七个合作者. 证据说明 vibe research 不是某个 trick 在一个问题上的巧合, 是可泛化的 protocol.
...
中绿: 反应理论和机制 8 篇. 这里有和 Tongji 本地 Liu Ren 组的合作. Coherent Absorption 分析 CDCC 里吸收的相干结构, 已发表 PRC 113, 054601. Channel couplings redirect 分析 channel coupling 如何把被吸收的 flux 从外部损失转移到融合, 已发表 PLB 877, 140479. Deletion Does Not Measure 说明在 projected dynamics 里删除一个通道不等于测量它的贡献. Exact Green's Function 严格构造耦合道 Green 函数并证明唯一性. Knockout quenching 是 solo, 从 double Feshbach projection 导出 composite-projectile 有效三体 Hamiltonian. IAV breakup 推广经典 IAV sum rules. 再加两篇草稿: 三体弹核的 inclusive breakup, 以及 dispersive CDCC 弹性有效相互作用 (w/ Liu, Ren).
...
三组加起来 16 篇, 四个月, 涉及三个子领域主要合作者 OSU Ohio Texas A&M 加 Tongji 本地.

核心观察: 16 篇横跨数值方法纯理论统计推断 EFT 多个方向. vibe research 的工作流不是某个特定技巧在某个特定问题上的巧合, 是可以跨越子领域边界的 protocol.
...
<strong>我怎么发现的:</strong> 跑 benchmark 看到一个量偏了约 $\pi$。几分钟内定位到问题, 因为我对物理正确的结果形态有预期。<em>(对实验组听众的同构版本: s-SNOM 反演给出的介电函数看起来"还行", 但虚部符号反了 / Kramers-Kronig 关系不满足; 或 pump-probe 全局拟合给出的衰减常数看起来合理, 但某一分量的 amplitude 符号搞错了。这类错误不会让程序崩溃, 只会给出错误的结果。)</em>
...
<!--
Central message: 把 vibe research 从抽象落到具体的 skill 工具箱上. 16 篇是 industrial 级产出不是 artisanal.

讲: 上一页是 what, 这一页是 how. 我的 skill 工具箱是个人品味的蒸馏, 每个人需要建自己的, 不可复制.

左栏规划和档案. research-planning 是每个新项目的入口. 生成三个文件: CLAUDE.md 给 Claude Code 的 persistent instructions, 用 imperative form 写 ("Use JAX" 不是 "the project uses JAX"). README.md 项目 public face 像 mini-paper. TODO.md phased tasks with checkboxes 从 Phase 0 literature survey 到 Phase 4 manuscript. 三个文件让每个新项目从一开始就有骨架. research-profile 是我的个人研究档案 wiki, 项目 / 论文 / 想法 / 失败 / 方法 / 合作者结构化互链, 自动注入每个新 session, 让 AI 一上来就知道我的研究脉络. todo 跨会话维持任务状态.

中栏文献和 debug. literature-wiki 是我的个人文献知识库, 读过的每篇论文结构化互链, 可以跨文献查询、发现矛盾、做综合, 是 raw PDF 和 LLM 之间持续累积的中间层. debug-physics-first 是我对 Expert Filter 的 automate. Rule Zero 是听用户指令, 然后在任何复杂 debug 之前先写 5 行 invariance test. Translation rotation parity permutation time-reversal 这些对称性必须精确成立. 如果违反了 bug 在代码里不在物理里. 这比 trial-and-error 快十倍.

右栏写作和报告. prc-writing prl-writing 期刊专用, 知道各自格式文献风格 INSPIRE-HEP 优先级. review-writing Hallmarks-style framework, literature-wiki 优先做文献支撑. slidev-talk 是今天这份 slides 的生成器. 我想强调: 今天这份 deck 本身就是 vibe research 的产物. slidev-talk + Claude 合作生成. 我做 judgment, 机器做 implementation.

关键观察: 16 篇不是 16 次灵感是一条流水线跑 16 次. 上个月不能做的事下个月可能就能做, 因为 pipeline 在自己进化.
...
# <span style="color: var(--pencil-blue)">●</span> 落到实验组的日常：机会地图
## 把四个前提条件对照到你们的工作流环节

<div class="mt-3 text-sm">

**重申前提**: workflow 要在一个环节 work, 需要 (1) 方法骨架成熟 (2) 代码规模 1k–10k 行 (3) 强可验证物理约束 (4) benchmark / 参考解密度足够。
...
| 实验组候选环节 | ① 骨架成熟 | ② 代码规模 | ③ 可验证约束 | ④ Benchmark 密度 | 判断 |
|---|:---:|:---:|:---:|:---:|:---:|
| **s-SNOM / near-field 图像管线** (去背景、demodulation、反演) | 高 | 中 | KK 关系、已知材料标样 | 多 | 🟢 可直接用 |
| **pump-probe 时间序列全局拟合** (多分量衰减、振荡成分) | 高 | 小到中 | 因果律、符号约束、极限行为 | 多 | 🟢 可直接用 |
| **FDTD / Lumerical / MEEP 参数扫描 wrapper** | 高 | 中 | 能量守恒、对称性、解析极限 | 多 | 🟢 可直接用 |
| **DFT / TDDFT / BSE 辅助计算 wrapper** | 高 | 中 | sum rule、已知材料能带 / 激子 | 多 | 🟢 可直接用 |
| **仪器控制 / 自动化采集脚本** (锁相、延时台、偏振扫描) | 中到高 | 小到中 | 实时可视化、回路验证 | 硬件相关 | 🟡 需谨慎 (硬件安全) |
| **图像批处理 / 论文图表生成** | 高 | 小 | 人眼即可验证 | 多 | 🟢 立即开始 |
| **新型反演算法 / 新方法开发** (非标准形式) | 低到中 | 大 | 弱 | 稀 | 🔴 暂不适用 |

</div>

<div class="box-gap mt-3 text-sm">

**关于这张表:** 具体打分需要各位老师自己修正——我只是外部观察者。但我敢肯定的是: **绝大多数实验组日常的"摩擦层"(数据清洗 / 格式转换 / 批量拟合 / 参数扫描 / 出图)** 都落在绿色区域, 却仍在由学生手写一次性脚本完成。

</div>

<div class="box-idea mt-3 text-sm">

**绿色环节:** 值得用一个周末在 agentic workflow 下试一次, 用一个你烂熟的小问题 (比如把过去一个学生写了两周的某个拟合脚本, 让 AI 重写一遍看结果)。<br>
**黄色环节 (仪器控制):** 真机调试前必须在 dry-run / simulator 下跑通, 硬件安全不可外包。<br>
**红色环节:** 现阶段 workflow 不适用, 但仍值得关注下一代工具。
...
<!--
Central message: 把四个结构性前提作为一个框架, 对实验组的各个环节打分. 大多数"摩擦层"其实都落在可以直接交给 agentic AI 的区域.

讲: 视角拉远. 我的工作在核反应里. 但四个前提不是核物理独有的, 它是任何计算 / 数据密集型子环节都能套的结构性清单. 我列了七个你们日常会遇到的环节, 括号里的打分请各位老师修正. 真正的用法是拿这张表回去, 对自己组里每个学生每周花最多时间的那些环节打分, 然后优先从绿色区域开始试. 仪器控制标黄是因为硬件安全要单独考虑, 真机调试前必须 dry-run.
...
实验组面对的是另一种形态的"人手不够": 数据涨得比分析管线快, 仿真跑得比参数空间大, 学生时间消耗在一次性脚本而非物理判断上。<br>
工具的杠杆在这里只会更大, 不会更小。
</div>
...
所以我留给这个房间的问题不是"要不要用",<br>
而是: 在你们组的工作流里, 它会从哪个环节先动起来?
...
<!--
Central message: 核物理是成熟领域里一个老兵的实证; 实验组的"人手不够"是另一种形态——数据比分析管线涨得快, 仿真比参数空间大. 把问题以"从哪个环节先动起来"的方式留给房间.

收束: 我想把这场报告收在一个问题上. 我所在的核反应被长期称作"成熟", 但"成熟"常常不是问题被回答了, 而是人手不够. 16 篇论文是一个早期证据 —— 即使在老兵领域, 工具一出现就能让停滞的问题重新动起来. 实验组的"人手不够"是另一种形态: 你们每个人都有大量时间花在一次性脚本、格式转换、批量拟合、图像清洗上, 而不是花在物理上. 杠杆在这里只会更大. 问题不是"要不要用", 而是"从哪个环节先动起来"——我的建议是从上一页那张表的绿色区域开始.
...
| 8 | 2603.24253 | 2026-03-25 | Deletion Does Not Measure (CC Dynamics) | Jin Lei, Liu ⭐ PRC accepted |
| 9 | 2604.00471 | 2026-04-01 | Exact CC Green Function | Liu, Jin Lei, Ren |
| 10 | 2604.05600 | 2026-04-07 | Channel couplings redirect absorbed flux | Liu, Jin Lei, Ren ⭐ PLB 877, 140479 |
...
2025 年 12 月至 2026 年 4 月. 11 篇上 arXiv (Fisher-information / info-geometry 等 5 篇草稿未上), 3 篇 PRC + 1 篇 PLB 已发表, 1 篇 PRC 已接收. Solo × 9, 同济本地组 × 5, w/ Furnstahl × 2, w/ Hu+Phillips+Furnstahl × 1 (重叠计).
</div>
...
<em>3 篇 PRC + 1 篇 PLB 发表, 1 篇 PRC 接收, 其余在同行评审中。请直接读 arXiv (清单见 B1)。加速不降低审稿标准是五条原则中最硬的一条。</em>
</div>
