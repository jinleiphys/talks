...
<Cast src="/casts/demo-kb.cast" :clicks="$clicks" :speed="1" poster="npt:0:04" :markers="[[13.00, '定位到 0710.4193'], [18.30, '顺着引用图走两个方向']]" />

<!--
真实录制。Claude Code，Opus 5，23 秒。检索路径是排练过的，命中和数字是当场查出来的。

前两下停在定位和引用图，第三下不停，一路放到底，末尾那两行才是第三张卡：原始摘要归原始摘要，机器生成的 Key numbers 归机器。
答案是最后一秒半一次刷出来的，所以第三个停顿点没法卡在中间，只能让它跑完。
...
<Cast src="/casts/demo-n90zr.cast" :clicks="$clicks" :speed="1.5" :markers="[[7.10, 'KD02 参数 · ap=0'], [26.40, '收敛 · 1301.64017 mb'], [30.10, 'EXFOR · 只有 8 / 10 / 24 MeV']]" />

<!--
真实录制。Claude Code，Opus 5，原始约 44 秒。命令是排练过的，数字是当场算出来的，别在台上说成它自己想出来的流程。

前三下停在参数、收敛、EXFOR 三张卡上，第四下不停，一路放到底，四条结论一起出来。
（四条结论是最后一次刷出来的，卡不住中间。）

收敛那一段值得讲：基准卡给出 σ_R = 1301.63782 mb；把 hcm 从 0.1 减半到 0.05，
jtmax 从 60 增到 120，再把 absend 从 0.001 放到 0，得到 1301.64017 mb，相对变化约 1.8e-6。
