...
2026-10-08，45 分钟 + 提问。主线是 agent，FUSION 是证据。22 页正文 + 4 页备用，每页末尾的注释块是讲稿（中心信息、讲什么、时间、转场）。
...
**演示一（第 11 页，断网）**：交给它的原话

> 这篇论文（Abu-Ibrahim 等，PRC 77, 034607）在我们的知识库里是哪一篇？它引了谁，谁引了它？摘要里有什么可以直接核对的数字？

要点出的三个节点：定位到 `0710.4193`；引它 8 篇、被引 5 篇；p+12C 40 MeV σ_R = 432 mb。切屏前先关 Wi-Fi。
...
**两段都必须先录一份剪好的备份**，命名 `demo-kb.mp4`、`demo-n90zr.mp4` 放在本目录。现场 30 秒没实质进展就切录屏，不解释，不道歉，不现场调试。
